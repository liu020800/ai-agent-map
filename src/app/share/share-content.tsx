      ...
      <Section className="relative z-10" spacing="sm">
        <PageShell width="wide">
          <div className="mb-8 text-center">
            <p className="title-font text-[11px] uppercase tracking-[0.34em] text-cyan-300">Agent Passport</p>
            <h1 className="title-font mt-4 text-4xl font-black text-white sm:text-6xl">你的 AI Agent 身份卡已生成</h1>
            <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-white/62">
              这是一张属于你的全国 AI 信号身份卡。生成后会保存记录，刷新页面不会再次调用生图模型。
            </p>
          </div>

          {loading && (
            <div className="rounded-3xl border border-cyan-300/14 bg-black/40 p-10 text-center text-cyan-100/70">正在读取已保存身份卡...</div>
          )}

          {!loading && !card && (
            <div className="mx-auto max-w-3xl rounded-3xl border border-cyan-300/14 bg-black/45 p-6 text-center backdrop-blur-xl sm:p-8">
              <h2 className="title-font text-3xl font-black text-white">没有找到身份卡数据</h2>
              <p className="mt-4 text-sm leading-7 text-white/62">请先完成 AI Agent 身份扫描，生成你的专属身份卡。</p>
          ...
          {card && (
            <div className="grid gap-6 lg:grid-cols-[minmax(0,0.95fr)_420px]">
              <div className="overflow-hidden rounded-[28px] border border-cyan-300/18 bg-black/55 p-3 shadow-[0_0_60px_rgba(34,211,238,0.14)] backdrop-blur-xl">
                {card.imageUrl ? (
                  <img src={card.imageUrl} alt={card.shareTitle} className="mx-auto max-h-[78vh] w-full rounded-[22px] object-contain" />
                ) : (
                  <div className="flex min-h-[520px] items-center justify-center rounded-[22px] border border-white/10 text-white/50">身份卡图片暂不可用</div>
                )}
              </div>

              <aside className="space-y-4">
                <div className="rounded-3xl border border-white/10 bg-black/45 p-5 backdrop-blur-xl">
                  <p className="title-font text-[10px] uppercase tracking-[0.28em] text-cyan-300/70">Card Info</p>
                  <h2 className="title-font mt-2 text-2xl font-black text-white">{card.nickname}</h2>
                  <p className="mt-2 font-mono text-sm text-cyan-100/80">{card.cardId}</p>
                  <div className="mt-4 space-y-2 text-sm text-white/68">
                    <p><MapPin className="mr-2 inline h-4 w-4 text-cyan-300" />{card.province}{card.city}</p>
                    <p><Sparkles className="mr-2 inline h-4 w-4 text-violet-300" />{card.tools.join(" / ")}</p>
                    <p>{card.signature || "用 AI 扩展自己的能力边界"}</p>
                    <p className="text-xs text-white/38">创建时间：{new Date(card.createdAt).toLocaleString("zh-CN")}</p>
                  </div>
                </div>
                ...
                <div className="rounded-3xl border border-white/10 bg-black/45 p-5 backdrop-blur-xl">
                  <p className="mb-3 text-sm font-semibold text-white">分享文案</p>
                  <p className="whitespace-pre-line rounded-2xl border border-white/10 bg-black/35 p-4 text-sm leading-7 text-white/75">{shareText}</p>
                </div>
                ...
                <p className="text-center text-xs text-white/35">AI 生成内容 · 仅供娱乐分享，不代表真实身份认证。</p>
              </aside>
            </div>
          )}
        </PageShell>
